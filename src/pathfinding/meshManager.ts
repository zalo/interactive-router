  ...
  for (const pair of pairs) {
    ...
    try {
      ...
      if (failNoPath <= 1) {
        ...
        // Check if NON_OBSERVABLE successors are being generated by running
        // a step-by-step search on the same inputs
        const si2 = new SearchInstance(mesh)
        si2.timeLimitMs = 100
        si2.ignoreObstacles = ignoreObstacles
        const events = si2.searchInit()
        let obsCount = 0, nonObsCount = 0, blocked = 0
        for (let step = 0; step < 20 && !si2.isSearchComplete(); step++) {
          const stepEvents = si2.step()
          for (const ev of stepEvents) {
            if (ev.successors) {
              for (const s of ev.successors) {
                if (s.type === "OBSERVABLE") obsCount++
                else nonObsCount++
              }
            }
            if (ev.node && step < 5) {
              console.warn(`[search] step${step}: root=${ev.node.root} poly=${ev.node.nextPolygon} f=${ev.node.f.toFixed(2)} g=${ev.node.g.toFixed(2)}`)
            }
          }
        }
        console.warn(`[search] First 20 steps: ${obsCount} observable, ${nonObsCount} non-observable successors, ${blocked} blocked`)
